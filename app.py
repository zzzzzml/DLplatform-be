...
def upload_attachment():
    ...
    # 创建实验文件夹结构（如果不存在）
    experiment_folder = f"实验{experiment_id}"
    experiment_path = os.path.join(os.getcwd(), experiment_folder)
    ...
    # 创建三个子文件夹
    subfolders = ['testdata', 'upload', 'testcode']
    for subfolder in subfolders:
        subfolder_path = os.path.join(experiment_path, subfolder)
        if not os.path.exists(subfolder_path):
            os.makedirs(subfolder_path)
    
    # 将文件保存到实验文件夹的upload子文件夹中
    filename = secure_filename(file.filename)
    file_extension = os.path.splitext(filename)[1]
    unique_filename = f"{uuid.uuid4()}{file_extension}"
    save_path = os.path.join(experiment_path, 'upload', unique_filename)
    file.save(save_path)
...
def publish_experiment_with_attachment():
    try:
        ...
        # 创建实验文件夹结构
        experiment_folder = f"实验{experiment.experiment_id}"
        experiment_path = os.path.join(os.getcwd(), experiment_folder)
